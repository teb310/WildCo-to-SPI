...
# E.g. If your first set of camera checks occured in April and May of 2021, write `"2021-04"|"2021-05"`.
  ...
  mutate(
    adult_females = case_when(
      ...
      sex_code == "F" & life_stage_code == "A,J" | 
        sex_code == "M,F" & life_stage_code == "A" | 
        sex_code == "M,F" & life_stage_code == "A,J" ~ as.numeric(str_extract(comments, "\\d(?= female| females)")))) %>%
  mutate(
...
image_data$tally <- image_data %>%
  select(adult_males:yearling_UC) %>%
  rowSums(na.rm = T)
